...
getHypoxiaExtent <- function(year, aggType, method, r, unit) {
  ...
  for (threshold in c("less0", "less2", "less4")) {
    ...
    p <- ggplot(HE) +
      ...
      geom_line(aes(time, median / n), color = "black") +
      geom_line(aes(time, idw / n), color = "blue", alpha = 0.7, size = 0.5) +
      ylim(c(0, 1)) +
      ylab("Hypoxia Extent") +
      theme_bw() +
  ...
  for (threshold in c("less0", "less2", "less4")) {
    ...
    pdf(sprintf("%s/results/%d_%s_%s_%d_extent_%s_km2.pdf", outputBaseName, year, aggType, method, r, threshold),
      width = 5, height = 3
    )
    print(p)
    dev.off()
...
getBasisReconstructions <- function(year, aggType, loggerID_) {
  folderName <- sprintf("./output/%d_%s_%s_%d/", year, aggType, "Reml", 10)
  model <- readRDS(paste0(folderName, "basisModelRes.rds"))
...
getDecompositionResults <- function(year, aggType, r, reverseFirst = FALSE, stvgm = FALSE) {
  ...
  folderName <- sprintf("%s/%d_%s_%s_%d/", outputBaseName, year, aggType, "Reml", r)
  ...
  print(plot(basis[, 1:min(r, 3)],
    xlab = "Time", nc = 3, yax.flip = FALSE, cex.axis = 1.5, cex.lab = 2, main = "", oma = c(5, 0, 2, 0),
    mar = c(0, 5.1, 0, 2.1)
  ))
  dev.off()
  ...
  for (year_ in yearList) {
    for (aggType_ in aggList) {
      p_rmse <- dplyr::filter(fullRes, aggType == aggType_, year == year_) %>%
        ggplot(data = .) +
        geom_boxplot(aes(x = factor(r), y = rmse, fill = method), size = I(0.5), position = position_dodge(width = 0.8), outlier.size = 0.5) +
      ...
      p_withBound <- dplyr::filter(fullRes, aggType == aggType_, year == year_, method != "IDW") %>%
        ggplot(data = .) +
        geom_boxplot(aes(x = factor(r), y = withinBoundRatio, fill = method), size = I(0.5), position = position_dodge(width = 0.8), outlier.size = 0.5) +
        xlab("Model Parameter") +
        ylab("CI Coverage") +
        theme_bw() +
        scale_fill_manual(
      ...
      prow <- plot_grid(
        ...
        p_withBound + theme(legend.position = "none"),
        align = "h"
      )
