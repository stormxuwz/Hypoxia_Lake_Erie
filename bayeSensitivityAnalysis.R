...
ggplot(data = diffRes) +
  geom_boxplot(aes(x = as.factor(parameter), y = rmse_diff), size = I(0.5), position = position_dodge(width = 0.8), outlier.size = 0.5) +
  xlab("Hyper parameter set") +
  ylab(TeX("$\\Delta$ RMSE")) +
  theme_bw()
dev.off()
...
ggplot(data = diffRes) +
  geom_boxplot(aes(x = as.factor(parameter), y = CI_coverage_diff), size = I(0.5), position = position_dodge(width = 0.8), outlier.size = 0.5) +
  xlab("Hyper parameter Set") +
  ylab(TeX("$\\Delta$ CI coverage")) +
  theme_bw()
dev.off()
