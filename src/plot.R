...
plot_gif <- function(year, aggType, method, r) {
  ...
  if (method == "idw") {
    predictions <- readRDS(
      sprintf(
        "%s/%d_%s_idw/trendPredictions.rds",
        outputBaseName, year, aggType
      )
    ) %>% reConstruct()
  ...
  tmp <- foreach(i = seq(startIdx, length(timeIdx), 1)) %dopar% {
    ...
    p <- baseMap + geom_tile(
      aes(longitude, latitude, fill = value),
      data = grid
    )
    p <- p + geom_point(
      aes(longitude, latitude, fill = value),
    ...
    p <- p + ggtitle(
      strftime(as.POSIXct(timeIdx[i]),
        tz = "America/New_York",
        usetz = TRUE
      )
    )

    png(sprintf("%s/hypoxiaSpatial_%d.png", folder, i),
...
plot_DO_temp <- function(data) {
  ...
  p <- p %>%
    dyAxis("y", label = varUnit[["DO"]]) %>%
    dyAxis("y2", label = varUnit[["Temp"]], independentTicks = TRUE) %>%
    dyRangeSelector(retainDateWindow = TRUE)
  return(p)
}
...
plot_wave_DO <- function(meta_available) {
  for (i in 1:nrow(meta_available)) {
    ...
    p1 <- ggplot() +
      geom_point(aes(Time, scale2Unit(DO)), data = DO_data) +
      geom_line(aes(time, scale2Unit(uc^2 + vc^2)), data = wave_data, color = "red")
    p2 <- ggplot() +
      geom_point(aes(Time, scale2Unit(DO)), data = DO_data) +
      geom_line(aes(time, scale2Unit(uc)), data = wave_data, color = "yellow")
    p3 <- ggplot() +
      geom_point(aes(Time, scale2Unit(DO)), data = DO_data) +
      geom_line(aes(time, scale2Unit(vc)), data = wave_data, color = "green")
    p4 <- ggplot() +
      geom_point(aes(Time, scale2Unit(DO)), data = DO_data) +
      geom_line(aes(time, scale2Unit(wvd)), data = wave_data, color = "blue")
