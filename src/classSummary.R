...
summary.basisModel <- function(
    trendModel,
    residualPrediction, parallel, totalSim, indMatrix = NULL) {
  ...
  hypoxiaAverage <- hypoxiaExtent_0

  for (i in 1:totalSim) {
    hypoxiaExtent_0[, i] <- res[[i]]$less0
    hypoxiaExtent_2[, i] <- res[[i]]$less2
    hypoxiaExtent_4[, i] <- res[[i]]$less4
    hypoxiaAverage[, i] <- res[[i]]$averageValue
  }

  hypoxiaSummary <- lapply(list(hypoxiaExtent_0, hypoxiaExtent_2, hypoxiaExtent_4, hypoxiaAverage), summaryStatistics)
  names(hypoxiaSummary) <- c("less0", "less2", "less4", "average_DO")
...
hypoxiaCount <- function(prediction) {
  ...
  averageValue <- rowMeans(prediction, na.rm = TRUE)

  res <- data.frame(
    less0 = hypoxiaExtent_0,
